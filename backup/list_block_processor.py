...
class ListBlockProcessor:
    ...
    def __post_list(
        ...
        whitespace_string = ParserHelper.repeat_string(
            ParserHelper.space_character, remaining_whitespace
        )
        return (
            container_level_tokens,
            f"{whitespace_string}{line_to_parse[after_marker_ws_index:]}",
            None,
        )
        # pylint: enable=too-many-arguments, too-many-locals
    ...
    def __process_eligible_list_start(
        ...
        if current_start_index < old_start_index:
            ...
            if len(current_container_blocks) > 1:
                ...
                POGGER.debug(
                    "adjusted_index>$ > parser_state.token_stack.indent_level>$ ) or not with_previous_list_bounds=$",
                    adjusted_index,
                    parser_state.token_stack[-2].indent_level,
                    with_previous_list_bounds,
                )
    ...
    def __adjust_line_for_list_in_process(
        ...
        padding_string = ParserHelper.repeat_string(
            ParserHelper.space_character, remaining_indent
        )
        return (
            f"{padding_string}{line_to_parse[start_index:]}",
            removed_whitespace,
        )

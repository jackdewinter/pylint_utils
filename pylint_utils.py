"""
Module to scan all python files and record any suppressed PyLint suppressions and
to optionally detect any that are no longer needed.
"""
import argparse
import glob
import json
...
class PyLintChecker:
    """
    Class to scan all python files and record any suppressed PyLint suppressions and
    to optionally detect any that are no longer needed.
    """
    ...
    def __log_level_type(argument):
        ...
        """
        if argument in PyLintChecker.available_log_maps:
            return argument
    ...
    def __parse_arguments(self):
        ...
        parser.add_argument(
            ...
            dest="log_level",
            action="store",
            default=PyLintChecker.__default_log_level,
            help="minimum level for any log messages",
            type=PyLintChecker.__log_level_type,
            choices=list(PyLintChecker.available_log_maps.keys()),
        )
        parser.add_argument(
            "--log-file",
            dest="log_file",
    ...
    def __decompose_valid_pyline_line(self, directive_text, directive_action):
    ...
    def __record_disabled_items(
        ...
        for next_item in list_of_items:
            ...
            if lowercase_next_item == PyLintChecker.__too_many_lines_item:
                continue
    ...
    def __check_contents_of_python_file(self, file_name, file_contents):
        ...
        for next_line in file_contents:
            ...
            if stripped_next_line.startswith(PyLintChecker.__pylint_suppression_prefix):
                pylint_directive = stripped_next_line[
                    len(PyLintChecker.__pylint_suppression_prefix) :
                ].strip()
                if pylint_directive.startswith(
                    PyLintChecker.__pylint_suppression_disable
                ):
                    collected_items = self.__decompose_valid_pyline_line(
                        pylint_directive, PyLintChecker.__pylint_suppression_disable
                    )
                    ...
                    PyLintChecker.__pylint_suppression_enable
                ):
                    collected_items = self.__decompose_valid_pyline_line(
                        pylint_directive, PyLintChecker.__pylint_suppression_enable
                    )
    ...
    def __remove_pylint_suppress_lines(self, content_lines, start_line, end_line):
    ...
    def __search_for_suppression_in_returned_list(
        self, logged_properties, modified_suppressions
    ):
    ...
    def __initialize_logging(self, args):
        ...
        if args.log_file:
            ...
            self.__new_handler.setLevel(
                PyLintChecker.available_log_maps[args.log_level]
            )
            base_logger.addHandler(self.__new_handler)
        else:
            base_logger.setLevel(PyLintChecker.available_log_maps[args.log_level])

    def __terminate_logging(self):
        if self.__new_handler:
            self.__new_handler.close()

    def __create_report_map(self, disabled_by_file_name_map):
        total_counts = {}
        # TODO replace for with .items
